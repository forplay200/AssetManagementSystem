...
export default function LoginPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const fallback = isSystemAdministrator(session.user)
        ? '/admin/users'
        : session.user?.role === 'user' && !session.user?.team ? '/workspace' : '/dashboard';
  ...
  return (
    <PublicAuthShell>
      ...
      {searchParams.get('reason') === 'account-inactive' && <div className="mt-5 rounded border border-red-400/20 bg-red-400/[0.07] px-3 py-2.5 text-sm text-red-200/80" role="status">This account is inactive. Contact a System Administrator for access.</div>}
