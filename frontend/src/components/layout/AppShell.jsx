...
import { accountRoleLabel, isSystemAdministrator } from '../../auth/roles';
...
const navigation = [
  { label: 'Dashboard', to: '/dashboard', icon: LayoutDashboard, permission: 'viewAsset', workspaceOnly: true },
  { label: 'Assets', to: '/assets', icon: FolderOpen, permission: 'viewAsset', workspaceOnly: true },
  { label: 'Search', to: '/search', icon: Search, permission: 'viewAsset', workspaceOnly: true },
  { label: 'Upload', to: '/upload', icon: Upload, permission: 'uploadAsset', workspaceOnly: true },
  { label: 'Team', to: '/team', icon: UsersRound, teamOnly: true, workspaceOnly: true },
  { label: 'Workspace setup', to: '/workspace', icon: Building2, workspaceOnly: true },
  { label: 'System users', to: '/admin/users', icon: UsersRound, permission: 'manageUsers', systemOnly: true },
];

function Sidebar({ onNavigate }) {
  const { can } = usePermissions();
  const { user } = useAuth();
  const systemAdministrator = isSystemAdministrator(user);
  return (
    <>
      <div className="border-b border-white/[0.08] px-5 py-[18px]">
        <BrandMark />
      </div>
      {!systemAdministrator && <WorkspaceSwitcher onNavigate={onNavigate} />}
      <nav className="flex-1 space-y-1 px-3 py-5" aria-label="Main navigation">
        <p className="mb-3 px-3 font-mono text-[10px] uppercase tracking-[0.16em] text-zinc-600">{systemAdministrator ? 'Platform' : 'Workspace'}</p>
        {navigation.filter((item) => (!item.permission || can(item.permission)) && (!item.teamOnly || user?.team) && (!item.systemOnly || systemAdministrator) && (!item.workspaceOnly || !systemAdministrator)).map(({ label, to, icon: Icon }) => (
          <NavLink
            key={to}
            to={to}
            onClick={onNavigate}
...
export default function AppShell() {
  ...
  useEffect(() => {
    if (!systemAdministrator && user?.team?.id) refreshTeam();
  }, [systemAdministrator, user?.team?.id, refreshTeam]);
    ...
    <div className="min-h-screen bg-aether-base text-zinc-200">
      ...
      <div className="md:pl-60">
        <header className="sticky top-0 z-20 flex h-[72px] items-center gap-3 border-b border-white/[0.08] bg-aether-base/85 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
          ...
          {can('viewAsset') ? <form className="relative max-w-xl flex-1" onSubmit={submitSearch}>
          ...
          </form> : <div className="min-w-0 flex-1"><p className="font-display text-sm font-semibold text-zinc-200">Platform administration</p><p className="mt-0.5 font-mono text-[9px] uppercase tracking-wider text-zinc-600">Account oversight</p></div>}
          <div className="ml-auto flex items-center gap-3 border-l border-white/[0.08] pl-3 sm:pl-5">
            ...
            <Link to="/profile" className="hidden min-w-0 sm:block">
              ...
              <p className="font-mono text-[10px] uppercase text-zinc-500">{systemAdministrator ? accountRoleLabel(user?.accountRole || user?.role) : user?.teamRole || user?.role || 'user'}</p>
            </Link>
